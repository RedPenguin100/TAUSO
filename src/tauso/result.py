...
def save_results_organism(
    df: pd.DataFrame, organism: str, experiment_name: str, algorithm: str
):
...
def save_results_on_target(df: pd.DataFrame, experiment_name: str, algorithm: str):
    csv_path = (
        EXPERIMENT_RESULTS / f"{experiment_name}/antisense_results/{algorithm}.csv"
    )
