...
@njit
def get_trna_dict():
    S_Cerevisiae_trna_anticodon_copy = Dict.empty(
        key_type=types.string, value_type=types.int64
    )
...
def is_nucleotide_wobble(codon_third: chr, anticodon_first: chr) -> bool:
    return (
        1.0 - nucleotide_wobble(codon_third, anticodon_first) > 1e-6
    )  # Lowest Wobble yields 1e-4 so this is safe
...
def calculate_tai(seq: str) -> float:
    ...
    np_weights = np.array(weights) / np.max(
        weights
    )  # normalize so number is between 0 and 1
