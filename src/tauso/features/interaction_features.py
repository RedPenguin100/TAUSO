...
def internal_fold_gymnosis(seq_internal_fold, transfection_gymnosis):
    ...
    self-fold energy ``seq_internal_fold`` (DNA-parameter self-fold MFE, <= 0) on gymnosis rows and 0
    ...
    if transfection_gymnosis is None:
        return seq_internal_fold * 0.0
    is_gymnosis = transfection_gymnosis.fillna(0.0)
    return seq_internal_fold.where(is_gymnosis > 0, 0.0)
