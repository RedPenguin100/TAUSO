...
def test_regression():
    ...
    try:
        ...
        if not os.path.exists(paths['fasta']):
            pytest.skip(f"Genome FASTA not found at {paths['fasta']}. Run 'tauso setup-genome' to enable this test.")
        print(f"Target Genome: {paths['fasta']}")
    except Exception as e:
        pytest.fail(f"Error locating genome data: {e}")
    ...
    try:
        ...
        pytest.fail(f"CRITICAL FAILURE during search: {e}")
