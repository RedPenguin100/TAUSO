...
def test_partition_and_merge_backbone_features(small_df, tmp_path):
    ...
    for k in range(2):
        ...
        assert (part_dir / "po_percentage.csv").exists(), f"Partition {k} should have saved po_percentage.csv"
        assert len(pd.read_csv(part_dir / "po_percentage.csv")) == 5

    merge_partitions(main_dir, n_partitions=2, index_col=index_col)

    for feature in ["po_percentage", "ps_end_score", "max_consecutive_PO"]:
