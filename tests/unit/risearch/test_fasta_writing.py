...
def test_rc_rna_equivalence(aso_seq):
    query = get_antisense(aso_seq)
    bio = str(Seq(query).reverse_complement_rna())
    fast = get_antisense_rna(query)
...
def test_fasta_block_content_matches(aso_seq):
    ...
    bio_block = f">{query_id}\n{Seq(query).reverse_complement_rna()}\n"
    fast_block = f">{query_id}\n{get_antisense_rna(query)}\n"
...
def test_multi_query_fasta_format():
    ...
    for query_id, query in pairs:
        ...
        lines.append(get_antisense_rna(query))
    new_content = "\n".join(lines) + "\n"
