...
Faithful transcription of ClinASO's scoring in notebooks/competitors/ClinASO/asodesigner/scr/_4.py
(github.com/wusuw/ClinASO; Chen et al., Mol Ther Nucleic Acids 37(2):102933, 2026), using
ClinASO's own position weight matrix (notebooks/competitors/ClinASO/asodesigner/text/human_rnaseH_pwm.txt,
...
def _load_pwm(path: Path = _PWM_PATH) -> dict:
    ...
    if not dic:
        raise RuntimeError(
            ...
            "Initialize the submodule: `git submodule update --init notebooks/competitors/ClinASO`."
        )
    return dic
