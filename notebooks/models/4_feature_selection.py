...
def rank_features(train_df, features):
    ...
    for j in range(len(features)):
        ...
        importance[j] = rmse(model.inplace_predict(X)) - base      # permuting an important feature raises RMSE
        X[:, j] = col
...
def main():
    ...
    for tr, va in common.gene_cv_folds(trv):
        ranking = rank_features(tr, features)
    ...
    final_ranking = rank_features(trv, features)                   # RMSE ranking over all train+val
