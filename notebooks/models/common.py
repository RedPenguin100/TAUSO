...
def load_dataset(use_cache=True):
    ...
    `use_cache=False` reads only what the feature pipeline wrote, ignoring the downloaded cache.
    """
    df, features = load_and_validate_final_data(version="oligo", use_cache=use_cache)
...
def mean_std(scores_list):
    ...
    return {
        m: (float(np.mean([s[m] for s in scores_list])), float(np.std([s[m] for s in scores_list]))) for m in METRICS
    }
