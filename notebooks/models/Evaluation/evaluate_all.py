...
ONCE with load_competition=True so every COMPETITION score (oligo_ai_score, PFRED, OligoWalk,
sfold, miRanda) is a column on each split, then scores models and competitors side by side
with Spearman / Top-1% / Top-5% inhibition / NDCG@5,10 / precision@5,10 / MAE / RMSE.

Usage:
    python -u -m notebooks.models.Evaluation.evaluate_all \\
        --models-dir notebooks/models/SimpleModels \\
        --out-dir    notebooks/models/SimpleModels/breakdown
"""
import argparse
...
def main():
    ...
    parser.add_argument("--models-dir", type=Path, default=Path("notebooks/models/SimpleModels"),
                        help="Directory containing per-config subdirs with trained models "
                             "(produced by train_model.py)")
    parser.add_argument("--extra-model", type=Path, action="append", default=[],
                        help="Path to an additional trained model not under --models-dir (repeatable).")
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("notebooks/models/SimpleModels/breakdown"),
    ...
    # 1. Discover models
    models = _discover_models(args.models_dir, args.model_glob)
    ...
    if not models:
        print(f"No models found under {args.models_dir} matching '{args.model_glob}' "
