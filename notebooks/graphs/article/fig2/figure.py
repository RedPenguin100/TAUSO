...
def _caption(D):
    ...
    return (
        ...
        "**(a, b)** Median within-experiment and within-cohort Spearman vs measured knockdown (whisker "
        "= 95% bootstrap CI over screens); TAUSO leads both, intervals non-overlapping. **(c)** Per "
        "experiment, the actual knockdown of each method's top-5%-ranked ASOs "
        "(violin = distribution across experiments, bar = IQR, dot = median), from the random floor to "
        "the best possible. **(d)** Per-chemistry within-experiment Spearman (whisker = 95% bootstrap "
        "CI over screens); TAUSO's margin over OligoAI is largest on cEt. **(e)** Parsimony: retraining "
        "on the top-K gain-ranked features, "
