...
MODEL_FEATURES = (
    Path(__file__).resolve().parents[3] / "src/tauso/inference/model/tauso_score_v1.features.txt"
)
...
def model_features() -> list[str]:
    ...
    return [line.strip() for line in MODEL_FEATURES.read_text().splitlines() if line.strip()]
...
def main():
    ...
    print(f"\nall {len(table)} features -> {OUT_CSV.relative_to(Path.cwd())}")
